...
public class ImmunityGUI extends GUI {
	...
	public ImmunityGUI(Player player, HexAttributes _plugin, HPlayer _hplayer)
    {
        super(_plugin.lang.immunity + " Attributes", 54, _plugin); // Must be multiple of 9!
        
        if (plugin.b_immGui == false) {
        	plugin.b_immGui=true;
        	//plugin.log("Registering Events: " + "Immunity Attributes");
	...
	public void addItems(HPlayer _hplayer)
    {
        ...
        // -- list out attributes
        int guiSlot = 0;
        for (Immunity a : plugin.configAttributes.types.immunity) {
