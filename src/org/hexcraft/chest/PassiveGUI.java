...
public class PassiveGUI extends GUI {
	...
	public PassiveGUI(Player player, HexAttributes _plugin, HPlayer _hplayer)
    {
        super(_plugin.lang.passive + " Attributes", 54, _plugin); // Must be multiple of 9!
        
        if (plugin.b_pasGui == false) {
        	plugin.b_pasGui=true;
        	//plugin.log("Registering Events: " + "Passive Attributes");
	...
	public void addItems(HPlayer _hplayer)
    {
        ...
        // -- list out attributes
        int guiSlot = 0;
        for (Passive a : plugin.configAttributes.types.passive) {
