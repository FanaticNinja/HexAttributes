...
public class StrikeGUI extends GUI {
	...
	public StrikeGUI(Player player, HexAttributes _plugin, HPlayer _hplayer)
    {
        super(_plugin.lang.strike + " Attributes", 54, _plugin); // Must be multiple of 9!
        
        if (plugin.b_strGui == false) {
        	plugin.b_strGui=true;
        	//plugin.log("Registering Events: " + "Strike Attributes");
	...
	public void addItems(HPlayer _hplayer)
    {
        ...
        // -- list out attributes
        int guiSlot = 0;
        for (Strike a : plugin.configAttributes.types.strike) {
