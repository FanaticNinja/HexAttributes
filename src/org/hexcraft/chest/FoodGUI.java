...
public class FoodGUI extends GUI {
	...
	public FoodGUI(Player player, HexAttributes _plugin, HPlayer _hplayer)
    {
        super(_plugin.lang.food + " Attributes", 54, _plugin); // Must be multiple of 9!
        
        if (plugin.b_fooGui == false) {
        	plugin.b_fooGui=true;
        	//plugin.log("Registering Events: " + "Food Attributes");
	...
	public void addItems(HPlayer _hplayer)
    {
        ...
        // -- list out attributes
        int guiSlot = 0;
        for (Food a : plugin.configAttributes.types.food) {
        	...
        	assAttrib.add(info);
        	assAttrib.add(ChatColor.GRAY + a.info);
        	assAttrib.add(ChatColor.RED + "Left-Click to Add");
			...
			if (plugin.config.bUsePerms ==  true) {
				if (!_hplayer.getPlayer().hasPermission("hexattributes.type." + a.name)) {
					...
					if (plugin.config.bHideAttributesWithNoPerms == false) {
						...
						assAttrib.add(ChatColor.RED + "Not Unlocked");
